...
Primary evidence: GraFT (arXiv:2609.03892), PruneGround (EMNLP 2026 main; arXiv:2606.31148), SparseTalk (arXiv:2609.15137), SceneGraphGrounder, SeGPruner, and peer-reviewed active-perception grounding.

**Novelty boundary:** generic language-conditioned pruning, unnamed-anchor discovery, target-anchor decomposition, prune-then-render, object-aware sparsification, or generic context retention are not defensible novelty claims. PruneGround and SparseTalk materially narrow these claims.
...
### 7. OLT-QA Seed Retrieval Front End
...
## Evidence update — 2026-09-23
- **SparseTalk (arXiv:2609.15137):** submitted 2026-09-14. It studies post-hoc sparsification for 3D VQA on ScanQA and MV-ScanQA. At 256 tokens it retains 0.80% of SplatTalk's 32,076-token inference input and 0.332% of the mean dense Gaussian field while reducing decoded-feature memory 125x. Its object-based selector distributes budget across object instances and retains background context.
- SparseTalk is question-independent and operates on Gaussian semantic tokens, so it does not directly solve this thesis's question-conditioned pre-render object selection. It nevertheless strongly supports the premise that dense full-scene evidence is redundant and that object/context coverage matters.
- No official SparseTalk code repository was verified in today's search; treat implementation transfer as conceptual until code appears.
- Novelty boundary tightened: generic object-aware sparsification or context retention is no longer sufficient. The key test is QA-conditioned named+unnamed support retention versus question-independent coverage and grounding-oriented pruning at matched object budgets.
...
`seed retrieval -> relation-conditioned scene-wide support candidates -> QA-utility allocation + small context reserve -> optional uncertainty expansion -> PoseAlign-T directional refinement -> selective Abstract3D render -> <=7B VLM`.

Reason: SparseTalk strengthens the empirical case for aggressive sparsity in 3D QA but leaves selection question-independent. The remaining defensible and testable distinction is whether QA-conditioned support selection achieves a better QA-vs-object-budget frontier than both grounding-oriented pruning and question-independent object coverage on ScanQA/SQA3D.
